...
/**
 * Active Schools Page - Shows only active schools with search functionality
...
$PAGE->set_url('/theme/remui_kids/active_schools.php');
$PAGE->add_body_classes(['limitedwidth', 'page-myactiveschools']);
$PAGE->set_pagelayout('mycourses');

$PAGE->set_pagetype('activeschools-index');
$PAGE->blocks->add_region('content');
$PAGE->set_title('Active Schools - Riyada Trainings');
$PAGE->set_heading('Active Schools');
...
// Handle AJAX request for filtered active school data
if (isset($_GET['action']) && $_GET['action'] === 'get_active_schools') {
    ...
    try {
        ...
        if ($company_table_exists) {
            ...
            // Build the query for active schools only
            $where_conditions = array('suspended = 0');
            $params = array();
        ...
        error_log("Active schools AJAX error: " . $e->getMessage());
...
// Get initial active school statistics
try {
    $company_table_exists = $DB->get_manager()->table_exists('company');
    ...
    error_log("Active schools page error: " . $e->getMessage());
    $total_schools = 0;
    $active_schools = 0;
    $suspended_schools = 0;
}
...
<div class="active-schools-container">
    <!-- Header Section -->
    <div class="header-section">
        <div class="header-content">
            <div class="header-text">
                <h1 class="header-title">Active Schools</h1>
                <p class="header-subtitle">View and manage all active schools in the system</p>
            </div>
    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <div class="stat-description">Currently active schools</div>
        </div>
        ...
        <div class="search-form">
            <div class="search-input-group">
                <input type="text" name="search" id="searchInput" class="search-input" 
                       placeholder="Search active schools by name, shortname, or city..." 
    ...
    <div id="loadingIndicator" class="loading-indicator" style="display: none;">
        ...
        <span>Loading active schools...</span>
    </div>
...
<script>
// AJAX Search functionality for active schools
document.addEventListener('DOMContentLoaded', function() {
    ...
    // AJAX function to fetch active school data
    function fetchActiveSchools(page = 0) {
        showLoading();
        
        const params = new URLSearchParams();
        params.set('action', 'get_active_schools');
        ...
        fetch(`<?php echo $CFG->wwwroot; ?>/theme/remui_kids/active_schools.php?${params.toString()}`)
            .then(response => response.json())
            .then(data => {
                hideLoading();
                if (data.error) {
                    console.error('Error:', data.error);
                    showError('Error loading active schools: ' + data.error);
                    return;
                }
            ...
            .catch(error => {
                ...
                showError('Network error: Unable to load active schools data');
            });
    }
            ...
            tableBody.innerHTML = `
                <tr>
                    <td colspan="6" style="text-align: center; padding: 40px; color: #6c757d;">
                        No active schools found matching your search criteria.
                    </td>
                </tr>
            `;
            return;
